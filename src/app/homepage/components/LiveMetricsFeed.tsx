  ...
  return (
    <div className="bg-[#ffffff] rounded-xl p-6 shadow-[#ffffff] border border-[#e2e8f0]">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-headline font-bold text-[#0f172a]">Live Transactions</h3>
      ...
      <div className="space-y-3 max-h-80 overflow-y-auto">
        {transactions.map((tx) => (
          <div key={tx.id} className="flex items-center justify-between p-3 bg-[#f8fafc] rounded-lg hover:bg-[#e2e8f0] transition-colors duration-200">
            <div className="flex items-center space-x-3 flex-1 min-w-0">
              <Icon 
                name={tx.status === 'success' ? 'CheckCircleIcon' : 'ClockIcon'} 
                size={20} 
                variant="solid" 
                className={tx.status === 'success' ? 'text-[#059669]' : 'text-[#d97706]'}
              />
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-[#0f172a] truncate">{tx.id}</p>
